...
class GameInstaller(path: String) {
     ...
     * Handles cases where:
     * 1. User selected the Data Files folder directly (contains Morrowind.esm or other ESM/BSA or named "Data Files")
     * 2. User selected the Morrowind root folder (contains 'Data Files' subfolder)
     * 3. User selected a parent folder containing nested 'morrowind' or 'Data' directory
     */
    fun resolveDataFilesDir(): File {
        if (!dir.exists()) return File(dir, DATA_NAME)

        // Case 0: The directory itself is named "Data Files"
        if (dir.name.equals(DATA_NAME, ignoreCase = true)) {
            return dir
        }
        ...
        // Case 3: Check 1 level deep for any folder containing game assets or a candidate subfolder
        try {
        ...
        // Case 4: Recursive search
        val recursive = findRecursive(dir, DATA_NAME)
    ...
    fun findIniFile(): File? {
        ...
        // Recursive search in dir and dir.parentFile
        val recDir = findRecursive(dir, INI_NAME)
        if (recDir != null) return recDir

        dir.parentFile?.let { p ->
            val recParent = findRecursive(p, INI_NAME)
            if (recParent != null) return recParent
        }
    ...
    fun check(): Boolean {
        ...
        if (findRecursive(dir, DATA_NAME) != null || findRecursive(dir, INI_NAME) != null) {
            return true
        }
            ...
            try {
                ...
                if (contents.isNotEmpty()) {
                    ...
                    if (output.isNotEmpty()) {
                        if (Constants.OPENMW_FALLBACK_CFG.isNotEmpty()) {
                            File(Constants.OPENMW_FALLBACK_CFG).parentFile?.mkdirs()
