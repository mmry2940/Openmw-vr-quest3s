...
class FragmentSettings : PreferenceFragment(), OnSharedPreferenceChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findPreference("game_files").setOnPreferenceClickListener {
            if (!permission.PermissionHelper.hasStoragePermission(activity)) {
                startActivity(Intent(activity, ui.activity.StoragePermissionActivity::class.java))
            } else {
        ...
        if (preference is android.preference.ListPreference) {
            if (key == "pref_uiScaling") {
                ...
                if (value.isNullOrEmpty()) {
                    ...
                    preference.summary = preference.entry ?: preference.value
                }
            } else {
                preference.summary = preference.entry ?: preference.value
            }
